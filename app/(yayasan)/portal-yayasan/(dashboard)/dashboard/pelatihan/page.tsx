...
import { api, Training } from '@/lib/yayasan/api';
...
export default function PelatihanPage() {
  ...
  async function handleSubmitForReview(id: string) {
    setSubmittingId(id);
    try {
      const updated = await api.submitTraining(id);
      setTrainings(prev => prev.map(t => (t.id === id ? { ...t, ...updated } : t)));
    } catch {
      // diamkan; status tetap
    } finally {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      const orgName = await api.getMe().then(me => me.Profile?.FullName ?? me.Email);
