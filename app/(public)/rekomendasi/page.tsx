    ...
    <Card className="hover:shadow-md transition-shadow border border-gray-100">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 flex-wrap mt-1">
              ...
              {item.city_name && (
                <span className="text-xs text-gray-400">· {item.city_name}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <CardContent className="space-y-2.5">
        <div className="flex items-center text-sm text-gray-600 gap-2">
          ...
          <span className="text-gray-400">dari posisi Anda</span>
        </div>
        <div className="flex items-center gap-2">
          {renderStars(item.predicted_rating)}
          <span className="text-xs text-gray-400">prediksi {Number(item.predicted_rating).toFixed(1)}</span>
        ...
        <Link href={`/lokasi-terapi/${item.id}`} className="block">
  ...
  return (
    <div className="max-w-md mx-auto py-12 px-4">
      ...
      <div className="bg-gray-50 rounded-xl border border-gray-100 p-4 mb-6 space-y-3">
        ...
        <div className="flex items-start gap-3">
          <Chrome size={16} className="text-gray-400 mt-0.5 flex-shrink-0" />
        ...
        <div className="flex items-start gap-3">
          <Settings size={16} className="text-gray-400 mt-0.5 flex-shrink-0" />
          <div className="text-xs text-gray-600">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <Input placeholder="-6.2088" value={latStr} onChange={e => setLatStr(e.target.value)} className="mt-1 text-sm" />
              </div>
              <div>
                <label className="text-xs text-gray-600 font-medium">Longitude</label>
                <Input placeholder="106.8456" value={lngStr} onChange={e => setLngStr(e.target.value)} className="mt-1 text-sm" />
              </div>
            </div>
...
export default function RekomendasiPage() {
  ...
  const [items, setItems] = useState<RecommendedLocation[]>([]);
  ...
  const fetchRecommendations = useCallback(async (lat?: number, lng?: number) => {
    ...
    setError(null);
    try {
      const res = await apiClient.public.recommendations.locations({ lat, lng, limit: 12 });
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">Pilih <strong>"Izinkan"</strong> pada dialog browser</p>
        </div>
      </div>
    );
  }
