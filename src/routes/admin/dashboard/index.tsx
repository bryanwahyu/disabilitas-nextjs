...
function DashboardPage() {
  // Setiap kartu dihitung dari endpoint berbeda; dipisah jadi query sendiri
  // supaya satu endpoint yang gagal tidak mengosongkan seluruh dashboard.
  const locationsParams = { limit: 1 };
  const { data: locations } = useQuery({
    queryKey: qk.admin.therapyLocations.list(locationsParams),
    queryFn: () => unwrapWithMeta(apiClient.adminTherapyLocations.list(locationsParams)),
  });

  const resourcesParams = { limit: 1 };
  const { data: resources } = useQuery({
    queryKey: qk.admin.resources.list(resourcesParams),
    queryFn: () => unwrapWithMeta(apiClient.adminResources.list(resourcesParams)),
  });

  const jobsParams = { limit: 1 };
  const { data: jobs } = useQuery({
    queryKey: qk.admin.jobs.list(jobsParams),
    queryFn: () => unwrapWithMeta(apiClient.adminJobs.list(jobsParams)),
  });

  const usersParams = { page: 1, page_size: 1 };
  const { data: users } = useQuery({
    queryKey: qk.admin.users.list(usersParams),
    queryFn: () => unwrapWithMeta(apiClient.adminUsers.list(usersParams)),
  });

  const contactsParams = { limit: 1000 };
  const { data: contacts = [] } = useQuery({
    queryKey: qk.admin.contacts.list(contactsParams),
    queryFn: () => unwrap(apiClient.adminContacts.list(contactsParams)),
  });
  ...
  const stats = {
    ...
    totalLocations: locations?.meta?.total ?? 0,
    totalJobs: jobs?.meta?.total ?? 0,
    totalContacts: contactList.length,
    unreadContacts: contactStats.unread,
    totalResources: resources?.meta?.total ?? 0,
  };
