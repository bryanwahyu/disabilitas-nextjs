...
import { api, PriceItem, PriceItemType } from '@/lib/yayasan/api';
import { qk } from '@/lib/query/keys';

export const Route = createFileRoute('/portal-yayasan/dashboard/harga/')({
  component: HargaPage,
});
...
function HargaPage() {
  ...
  const { data: prices = [], isPending: loading } = useQuery({
    queryKey: qk.yayasan.pricing.list(),
  ...
  const { mutate: createPrice, isPending: saving } = useMutation({
    mutationFn: (body: { item_type: PriceItemType; item_ref_id: string; harga_dasar: number }) =>
      api.createPrice(body),
    onSuccess: () => {
      setForm({ item_type: 'therapy_service', item_ref_id: '', harga_dasar: '' });
      setShowForm(false);
      qc.invalidateQueries({ queryKey: qk.yayasan.pricing.all() });
    },
    onError: (err: unknown) => {
  ...
  const { mutate: updatePrice } = useMutation({
    mutationFn: (vars: { id: string; harga_dasar: number }) => api.updatePrice(vars.id, vars.harga_dasar),
    onSuccess: () => {
      setEditId(null);
      qc.invalidateQueries({ queryKey: qk.yayasan.pricing.all() });
    },
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    createPrice({ item_type: form.item_type, item_ref_id: form.item_ref_id, harga_dasar: harga });
  }

  function saveEdit(id: string) {
    const harga = parseInt(editVal, 10);
    if (!harga || harga <= 0) return;
    updatePrice({ id, harga_dasar: harga });
  }
