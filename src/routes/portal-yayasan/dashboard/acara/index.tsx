...
export const Route = createFileRoute('/portal-yayasan/dashboard/acara/')({
  component: AcaraPage,
});
...
function AcaraPage() {
  ...
  const { data: events = [], isPending: loading } = useQuery({
    queryKey: qk.yayasan.events.list(),
    queryFn: () => api.getEventRequests(),
    select: res => res.data,
  });

  const { mutate: createEvent, isPending: saving } = useMutation({
    mutationFn: (body: object) => api.createEventRequest(body),
    onSuccess: () => {
      setForm(emptyForm);
      setShowForm(false);
      qc.invalidateQueries({ queryKey: qk.yayasan.events.all() });
    },
    onError: (err: unknown) => {
