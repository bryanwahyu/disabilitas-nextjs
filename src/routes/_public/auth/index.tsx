...
export const Route = createFileRoute('/_public/auth/')({
  /*
   * Judulnya sendiri, bukan judul default root.
   *
   * `noindex` disengaja dan sejalan dengan `Disallow: /auth` di robots.txt:
   * halaman masuk/daftar tidak punya nilai pencarian dan hanya menggerus
   * anggaran crawl. `follow` tetap dipakai supaya tautan di dalamnya diikuti.
   */
  head: () => ({
    meta: [
      { title: 'Masuk atau Daftar | DisabilitasKu' },
      {
        name: 'description',
        content: 'Masuk ke akun DisabilitasKu atau daftar sebagai pengguna, orang tua, terapis, atau yayasan.',
      },
      { name: 'robots', content: 'noindex, follow' },
    ],
  }),

  // `?redirect=` dibawa dari halaman yang memantulkan tamu ke sini, mis. slot
  // jadwal terapis. Dipakai setelah login berhasil (lihat komponen).
...
function AuthPage() {
  ...
  useEffect(() => {
    if (!authLoading && currentUser) {
      ...
      /*
       * `?redirect=` akhirnya dipakai.
       *
       * Tamu yang menekan slot jadwal terapis dibawa ke sini lebih dulu;
       * sebelumnya ia mendarat di /beranda dan harus mencari terapis itu lagi
       * dari nol. Hanya path internal yang diterima — nilai yang datang dari
       * URL tidak boleh bisa mengarahkan pengguna ke domain lain.
       */
      ...
      if (currentUser.role === 'instructor' || currentUser.role === 'employer') {
        navigate({ replace: true, to: '/dashboard' });
      } else {
        navigate({ replace: true, to: '/beranda' });
      }
    }
  }, [currentUser, authLoading, navigate, redirect]);
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="password"
                type="password"
                placeholder={isLogin ? 'Masukkan password' : 'Min. 8 karakter, ada huruf besar, angka, simbol'}
                value={password}
                onChange={(e) => {
